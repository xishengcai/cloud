...
check_version(){
  export VERSION_1=`docker version  --format="{{.Server.Version}}"`
  if [[ $VERSION_1 = "20.10.24" ]];then
  ...
  set -e
  echo "clean env"
  yum remove -y docker docker-common container-selinux docker-selinux docker-engine docker-buildx-plugin
